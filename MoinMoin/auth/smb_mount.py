...
class SMBMount(BaseAuth):
    ...
    def do_smb(self, request, username, password, login):
        ...
        if login:
            cmd = "sudo mount -t cifs -o user=%(user)s,domain=%(domain)s,uid=%(uid)d,dir_mode=%(dir_mode)s,file_mode=%(file_mode)s,iocharset=%(iocharset)s //%(server)s/%(share)s %(mountpoint)s >>%(log)s 2>&1"
        else:
